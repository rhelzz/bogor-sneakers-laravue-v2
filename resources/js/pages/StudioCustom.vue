<template>
    <div class="font-body bg-washi text-sumi antialiased">
        ...
        <section
            class="pattern-wave relative overflow-hidden px-6 pt-30 pb-12 lg:px-16"
        >
            <div
                class="pointer-events-none absolute -top-28 -right-20 h-64 w-64 rounded-full bg-matcha/15 blur-3xl"
            ></div>
            <div
                class="pointer-events-none absolute top-32 -left-24 h-72 w-72 rounded-full bg-sakura/20 blur-3xl"
            ></div>
            ...
            <div class="relative mx-auto max-w-7xl">
                <div class="grid items-end gap-10 lg:grid-cols-[1.15fr_0.85fr]">
                    <div>
                        <div
                            class="animate-slide-up mb-5 flex items-center gap-3"
                        >
                            <span
                                class="animate-pulse-soft h-2 w-2 rounded-full bg-matcha"
                            ></span>
                            <span class="text-xs tracking-[0.28em] text-hai"
                                >BOGORSNEAKER GUEST ORDER</span
                            >
                        </div>
                        <h1
                            class="font-heading animate-slide-up text-4xl leading-[0.95] font-bold tracking-tight text-sumi lg:text-6xl"
                        ...
                        <p
                            class="animate-slide-up mt-5 max-w-2xl leading-relaxed text-usuzemi"
                            style="animation-delay: 0.2s"
                        >
                            Halaman ini difokuskan untuk guest order custom
                            model: pilih model, atur material, personalisasi
                            artwork, lalu lanjutkan checkout tanpa akun.
                        </p>
                        <div
                            class="animate-slide-up mt-7 flex flex-wrap items-center gap-3"
                            style="animation-delay: 0.3s"
                        >
                            <span
                                class="rounded-full border border-sumi/15 bg-washi/90 px-4 py-2 text-[11px] font-semibold tracking-[0.12em] text-usuzemi"
                            >
                                LIVE SVG RENDER
                            </span>
                            <span
                    ...
                    <div
                        class="relative overflow-hidden rounded-3xl border border-sumi/10 bg-sumi px-6 py-7 text-washi shadow-xl"
                    >
                        <div
                            class="pointer-events-none absolute right-3 bottom-3 text-[78px] leading-none text-washi/8"
                        >
                            和
                        </div>
                        <p
                            class="vertical-text absolute top-5 right-4 text-[11px] tracking-[0.2em] text-washi/30"
                        >
                            スタジオ カスタム
                        </p>
                        <p class="text-xs tracking-[0.18em] text-washi/60">
                            STATUS
                        </p>
                        <p class="mt-2 text-2xl font-bold text-matcha">
                            Custom Model Guest Order
                        </p>
                        <p class="mt-4 text-sm leading-relaxed text-washi/75">
                            Pilih model, sesuaikan desain, lalu lanjutkan guest
                            checkout untuk proses order pembeli.
                        </p>
                        <div
                            class="mt-6 flex items-center gap-2 text-xs text-washi/70"
                        >
                            <span
                                class="h-2 w-2 animate-pulse rounded-full bg-matcha"
                            ></span>
                            Katalog terakhir:
                            <span class="font-semibold text-washi">{{
                                catalogTimeLabel
                            }}</span>
                        </div>
                    </div>
        ...
        <section class="pattern-grid px-6 pb-28 lg:px-16">
            <div
                class="mx-auto grid max-w-7xl gap-6 xl:grid-cols-[1.08fr_0.92fr]"
            >
                <div class="space-y-4">
                    <div
                        ref="viewerRef"
                    ...
                    >
                        ...
                        <div class="pointer-events-none absolute inset-0">
                            ...
                            >
                                <div
                                    ...
                                    role="button"
                                    tabindex="0"
                                    class="pointer-events-auto absolute top-0 left-0 leading-none font-black whitespace-nowrap select-none"
                                    :class="
                                        item.id === activeElementId
                                            ? 'ring-2 ring-matcha ring-offset-2 ring-offset-washi'
                                            : ''
                                    "
                                    :style="getTextElementStyle(item)"
                                    @pointerdown="
                                        startDragElement($event, item.id)
                                    "
                                ...
                                >
                                    <div
                                        class="transform-handle-connector"
                                    ></div>
                                    <button
                                        type="button"
                                    ...
                                    <button
                                        ...
                                        @pointerdown.stop.prevent="
                                            startScaleTransform($event, item.id)
                                        "
                                    >
                                        SCL
                                    </button>
                        ...
                        <div
                            class="pointer-events-none absolute bottom-4 left-4 rounded-lg border-l-4 border-matcha bg-washi/90 px-3 py-2 text-[11px] font-semibold text-usuzemi shadow-lg backdrop-blur-sm"
                        >
                            <p>
                                ID/WA: <span>{{ phone || '-' }}</span> | Nama:
                                <span>{{ name || '-' }}</span> | Size:
                                <span>{{ shoeSize || '-' }}</span>
                            </p>
                            <p class="mt-0.5">
                                O: <span>{{ selectedOutsoleShort }}</span> | M:
                                <span>{{ selectedMidsoleShort }}</span> | I:
                                <span>{{ selectedInsoleShort }}</span>
                            </p>
                        </div>

                        <div
                            v-if="!lazyCanvasReady"
                            class="absolute inset-0 z-20 flex items-center justify-center bg-sumi/80 text-center text-washi backdrop-blur-sm"
                        >
                            <div>
                                ...
                                <p class="mt-1 text-xs text-washi/70">
                                    Scroll ke area ini untuk memuat aset studio
                                    secara lazy.
                                </p>
                            </div>
                        </div>
                        ...
                        <div
                            v-if="isSyncing"
                            class="absolute inset-0 z-30 flex flex-col items-center justify-center gap-4 bg-sumi/85 text-center text-washi backdrop-blur-sm"
                        >
                            <div
                                class="h-12 w-12 animate-spin rounded-full border-4 border-washi/20 border-t-matcha"
                            ></div>
                            <div>
                                <p class="font-heading text-lg font-bold">
                                    Sinkronisasi Model
                                </p>
                                <p class="mt-1 text-sm text-washi/75">
                                    Sedang memuat aset SVG model terpilih...
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <p
                        class="rounded-2xl border border-matcha/25 bg-matcha/10 px-4 py-3 text-center text-sm font-semibold text-take"
                    >
                        Tips: drag elemen langsung di preview, klik bagian
                        sepatu untuk pilih aksen, lalu ubah warnanya cepat.
                    </p>
                </div>

                <div class="space-y-5">
                    <article
                        class="card-lift rounded-3xl border border-sumi/10 bg-washi/95 p-5 shadow-sm"
                    >
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <p
                                    class="text-[11px] font-black tracking-[0.14em] text-hai uppercase"
                                >
                                    Wizard Guest Checkout
                                </p>
                                <h2
                                    class="font-heading mt-1 text-xl leading-tight font-bold text-sumi"
                                >
                                    {{ currentStepMeta.title }}
                                </h2>
                                <p
                                    class="mt-1 text-xs leading-relaxed text-usuzemi"
                                >
                            ...
                            <div class="flex flex-col items-end gap-2">
                                <p
                                    class="rounded-full border border-sumi/15 bg-shironeri px-3 py-1 text-[11px] font-bold tracking-[0.08em] text-usuzemi"
                                >
                                    Langkah {{ currentStep }} /
                                    {{ TOTAL_STEPS }}
                                </p>
                                <div class="flex items-center gap-2">
                                    ...
                                    <span
                                        v-else
                                        class="text-[11px] font-semibold tracking-[0.06em] text-usuzemi uppercase"
                                    >
                                        Final
                                    </span>
                                </div>
                        ...
                        <div
                            class="mt-4 h-2 w-full overflow-hidden rounded-full bg-sumi/10"
                        >
                            <div
                                class="h-full rounded-full bg-matcha transition-all duration-300"
                    ...
                    >
                        ...
                        >
                            <h2
                                class="font-heading text-base font-bold text-sumi"
                            >
                                1. Pilih Family Model
                            </h2>
                            <button
                                type="button"
                        ...
                        <p
                            v-if="catalogError"
                            class="mb-3 rounded-xl border border-sakura/30 bg-sakura/10 px-3 py-2 text-xs font-semibold text-sakura"
                        >
                            {{ catalogError }}
                        </p>
                        ...
                        <div class="space-y-3">
                            <label
                                class="text-[11px] font-bold tracking-[0.12em] text-hai uppercase"
                            >
                                Tipe Sol / Family
                                <select
                            ...
                            <div>
                                <p
                                    class="mb-2 text-[11px] font-bold tracking-[0.12em] text-hai uppercase"
                                >
                                    Nomor Model
                                </p>
                                <div
                            ...
                            <div
                                v-if="currentModelMeta"
                                class="rounded-xl border border-sumi/10 bg-shironeri px-3 py-2 text-xs text-usuzemi"
                            >
                                <p>
                                    Layer aksen:
                                    <span class="font-bold text-sumi">{{
                                        currentModelMeta.layers.length
                                    }}</span>
                                </p>
                                <p>
                                    Layer pola:
                                    <span class="font-bold text-sumi">{{
                                        currentModelMeta.pattern_layers.length
                                    }}</span>
                                </p>
                            </div>
                        </div>
                    ...
                    <article
                        ...
                        class="card-lift rounded-3xl border border-sumi/10 bg-washi/95 p-5 shadow-sm"
                    >
                        <h2 class="font-heading text-base font-bold text-sumi">
                            2. Pilih Outsole (Sol Luar)
                        </h2>
                        <p class="mt-1 text-xs leading-relaxed text-usuzemi">
                            Outsole menentukan grip, ketahanan abrasi, dan
                            karakter pijakan saat dipakai harian.
                        </p>
                        ...
                        <div class="mt-4 grid gap-2">
                            ...
                            >
                                ...
                                <p
                                    class="mt-1 text-[11px] font-semibold text-take"
                                >
                                    {{ option.highlight }}
                                </p>
                            </button>
                    ...
                    >
                        <h2 class="font-heading text-base font-bold text-sumi">
                            3. Pilih Midsole (Sol Tengah)
                        </h2>
                        <p class="mt-1 text-xs leading-relaxed text-usuzemi">
                            Midsole menentukan tingkat bantalan, respons
                            pijakan, dan stabilitas kaki.
                        </p>
                    ...
                    >
                        <h2 class="font-heading text-base font-bold text-sumi">
                            4. Pilih Insole (Sol Dalam)
                        </h2>
                        <p class="mt-1 text-xs leading-relaxed text-usuzemi">
                            Insole berpengaruh besar ke kenyamanan, hygiene, dan
                            dukungan telapak kaki.
                        </p>
                    ...
                    >
                        <h2 class="font-heading text-base font-bold text-sumi">
                            5. Personalisasi Teks & Artwork
                        </h2>
                        <p class="mt-1 text-xs leading-relaxed text-usuzemi">
                            Tambahkan teks identitas, logo, atau artwork favorit
                            untuk menyesuaikan karakter pesanan.
                        </p>
                        ...
                        <div
                            ...
                            :class="
                                isDropActive
                                    ...
                                    : 'border-sumi/25 bg-shironeri/70'
                            "
                        ...
                        >
                            <p class="text-sm font-semibold text-sumi">
                                Drop artwork ke sini
                            </p>
                            <p class="mt-1 text-xs text-usuzemi">
                                PNG, JPG, WEBP. Bisa upload banyak sekaligus.
                            </p>
                            <button
                                type="button"
                        ...
                        <div class="mt-4">
                            ...
                            <div
                                v-if="uploadedMedia.length === 0"
                                class="rounded-xl border border-sumi/10 bg-shironeri px-3 py-4 text-center text-xs font-semibold text-hai"
                            >
                            ...
                            <div
                                v-else
                                class="editor-scroll max-h-54 space-y-2 overflow-y-auto pr-1"
                            >
                                <article
                                    ...
                                    :class="
                                        media.id === selectedUploadId
                                            ? 'border-matcha/60 bg-matcha/10'
                                            : 'border-sumi/15 bg-shironeri/80'
                                    "
                                >
                                    <div class="flex gap-2">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <p
                                                class="truncate text-xs font-bold text-sumi"
                                            >
                                                {{ media.name }}
                                            </p>
                                            <p
                                                class="mt-0.5 text-[11px] text-usuzemi"
                                            >
                                                {{ media.naturalWidth }} x
                                                {{ media.naturalHeight }} px
                                            </p>
                        ...
                        <div
                            class="mt-4 rounded-xl border border-matcha/30 bg-matcha/10 p-3"
                        >
                            <div
                                class="flex items-center justify-between gap-2"
                            >
                                <p
                                    class="text-[11px] font-bold tracking-[0.12em] text-take uppercase"
                                >
                                    Sumber Palet
                                </p>
                                <p class="text-xs font-semibold text-usuzemi">
                    ...
                    >
                        <h2 class="font-heading text-base font-bold text-sumi">
                            6. Kontrol Cepat Teks & Logo
                        </h2>
                        <p class="mt-1 text-xs leading-relaxed text-usuzemi">
                            Edit langsung di preview seperti Canva: drag elemen
                            untuk geser, tarik handle pojok untuk scale, dan
                            handle atas untuk rotate.
                        </p>

                        <div
                            v-if="!activeElement"
                            class="mt-4 rounded-xl border border-dashed border-sumi/20 bg-shironeri px-3 py-4 text-center text-xs font-semibold text-hai"
                        >
                            Pilih elemen teks atau gambar di preview untuk mulai
                            edit.
                        </div>

                        <div v-else class="mt-4 space-y-3">
                            <div
                                class="flex items-center justify-between gap-2 rounded-xl border border-sumi/15 bg-shironeri px-3 py-2"
                            >
                                <p
                                    class="text-xs font-black tracking-[0.16em] text-usuzemi uppercase"
                                >
                                    {{
                            ...
                            <template v-if="activeElement.type === 'text'">
                                <label
                                    class="text-[10px] font-bold tracking-[0.08em] text-hai uppercase"
                                >
                                    Isi teks
                                    <textarea
                            ...
                            <p
                                class="rounded-xl border border-matcha/35 bg-matcha/10 px-3 py-2 text-xs font-semibold text-take"
                            >
                                Gunakan handle hijau di preview: handle
                                kanan-bawah untuk memperbesar/mengecilkan,
                                handle bulat atas untuk memutar elemen.
                            </p>
                    ...
                    >
                        ...
                        <div
                            class="editor-scroll mt-4 max-h-70 space-y-3 overflow-y-auto pr-1"
                        >
                            <p
                                v-if="layerIds.length === 0"
                                class="rounded-xl border border-sakura/30 bg-sakura/10 px-3 py-2 text-xs font-semibold text-sakura"
                            >
                            ...
                            >
                                <div
                                    class="flex items-center justify-between gap-2"
                                >
                                    <p class="text-sm font-bold text-sumi">
                                        Aksen {{ id }}
                                    </p>
                                    <input
                                        type="color"
                                ...
                                <div class="mt-2 border-t border-sumi/10 pt-2">
                                    <label
                                        class="flex items-center gap-2 text-xs font-semibold text-usuzemi"
                                    >
                                        <input
                                            type="checkbox"
                                            :checked="
                                                layerOutlines[id]?.active ??
                                                false
                                            "
                                            @change="
                                                onLayerOutlineToggleInput(
                                                    id,
                                                    $event,
                                                )
                                            "
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <input
                                                type="color"
                                                :value="
                                                    layerOutlines[id]?.color ??
                                                    '#000000'
                                                "
                                                class="mt-1 h-8 w-full cursor-pointer rounded-md border border-sumi/20"
                                                @change="
                                                    onLayerOutlineColorInput(
                                                        id,
                                                        $event,
                                                    )
                                                "
                                        ...
                                        >
                                            ...
                                            <input
                                                ...
                                                max="50"
                                                :value="
                                                    layerOutlines[id]?.size ?? 2
                                                "
                                                class="mt-1 h-8 w-full rounded-md border border-sumi/20 bg-white px-2 text-xs"
                                                @input="
                                                    onLayerOutlineSizeInput(
                                                        id,
                                                        $event,
                                                    )
                                                "
                    ...
                    <article
                        v-else
                        class="card-lift rounded-3xl border-2 border-matcha/45 bg-matcha/10 p-5 shadow-sm"
                    >
                        <h2 class="font-heading text-base font-bold text-sumi">
                            8. Guest Checkout & Export
                        </h2>
                        <p class="mt-1 text-xs leading-relaxed text-usuzemi">
                            Lengkapi data pembeli, aktifkan add-on layanan, lalu
                            proses file checkout untuk konfirmasi pesanan.
                        </p>

                        <div class="mt-4 grid gap-3">
                            <label
                                class="text-xs font-bold tracking-[0.08em] text-usuzemi uppercase"
                            >
                                Nama Lengkap
                                <input
                                    v-model="name"
                            ...
                            <div
                                class="space-y-2 rounded-xl border border-sumi/15 bg-shironeri p-3"
                            >
                                <label
                                    class="flex items-start gap-3 text-sm font-semibold text-sumi"
                                >
                                    <input
                                    ...
                                    <span>
                                        ...
                                        <span
                                            class="mt-0.5 block text-xs font-semibold text-usuzemi"
                                            >Prioritas antrean produksi (+{{
                                                formatCurrency(FAST_TRACK_FEE)
                                            }})</span
                                        >
                                    </span>
                                </label>
                                ...
                                >
                                    ...
                                    <span>
                                        ...
                                        <span
                                            class="mt-0.5 block text-xs font-semibold text-usuzemi"
                                            >Box eksklusif untuk gift-ready
                                            delivery (+{{
                                                formatCurrency(CUSTOM_BOX_FEE)
                                            }})</span
                                        >
                                    </span>
                                </label>
                            ...
                            <div
                                class="rounded-xl border border-sumi/15 bg-white p-3"
                            >
                                <p
                                    class="text-[11px] font-bold tracking-[0.12em] text-hai uppercase"
                                >
                                    Ringkasan Biaya (Dummy)
                                </p>
                                <div
                                    class="mt-2 space-y-1 text-sm text-usuzemi"
                                >
                                    <p
                                        class="flex items-center justify-between gap-2"
                                    >
                                        <span>Base Custom Pair</span>
                                        <span class="font-semibold text-sumi">{{
                                            formatCurrency(BASE_PRODUCT_PRICE)
                                        }}</span>
                                    </p>
                                    <p
                                        class="flex items-center justify-between gap-2"
                                ...
                                <div class="mt-3 border-t border-sumi/15 pt-3">
                                    <p
                                        class="flex items-center justify-between gap-2 text-sm font-black text-sumi"
                                    >
                                        <span>Total Estimasi</span>
                                        <span>{{
                                            formatCurrency(guestCheckoutTotal)
                                        }}</span>
                                    </p>
                                </div>
                            </div>
                            ...
                            >
                                {{
                                    isSaving
                                        ? 'Memproses Checkout...'
                                        : 'Proses Guest Checkout & Download'
                                }}
                            </button>
...
const wizardSteps: WizardStep[] = [
    ...
    },
    {
        ...
        subtitle:
            'Edit elemen langsung di preview dengan drag, rotate handle, dan scale handle.',
    },
    {
        id: 7,
    ...
    },
    {
        ...
        subtitle:
            'Finalisasi data buyer, add-on layanan, lalu proses checkout.',
    },
];
...
const outsoleOptions: MaterialOption[] = [
    {
        ...
        shortLabel: 'Rubber',
        description:
            'Paling populer karena awet, antiselip, dan tahan air untuk pemakaian harian.',
    ...
    },
    {
        ...
        shortLabel: 'TPU',
        description:
            'Material polimer yang lebih ringan dari karet namun tetap kuat dan stabil.',
    ...
    },
    {
        ...
        shortLabel: 'TPR',
        description:
            'Kombinasi karet mentah dan plastik dengan traksi yang baik untuk area outdoor.',
    ...
    },
    {
        ...
        shortLabel: 'PVC',
        description:
            'Efisien untuk produksi massal, cocok untuk kebutuhan value-oriented.',
...
const midsoleOptions: MaterialOption[] = [
    {
        ...
        shortLabel: 'EVA',
        description:
            'Busa sangat ringan, lembut, dan fleksibel. Populer untuk sepatu lari serta kasual.',
    ...
    },
    {
        ...
        shortLabel: 'PU',
        description:
            'Lebih padat dan stabil dibanding EVA dengan ketahanan bentuk yang baik.',
    ...
    },
    {
        ...
        shortLabel: 'Phylon',
        description:
            'Versi EVA yang lebih padat dengan respons bantalan lebih baik.',
...
const insoleOptions: MaterialOption[] = [
    {
        ...
        shortLabel: 'Lateks',
        description:
            'Memberikan keempukan maksimal dan dikenal sebagai bahan premium anti pegal.',
    ...
    },
    {
        ...
        description:
            'Mengikuti bentuk telapak kaki untuk distribusi tekanan yang lebih merata.',
    ...
    },
    {
        ...
        shortLabel: 'Gel',
        description:
            'Mendukung area arch support dan tumit untuk menurunkan impact titik tekan.',
    ...
    },
    {
        ...
        description:
            'Kombinasi modern yang lebih ramah lingkungan serta memiliki sifat antibakteri.',
...
const activeFolder = computed(() => {
    return (
        catalogFolders.value.find(
            (folder) => folder.key === activeFolderKey.value,
        ) ?? null
    );
});
...
const currentModelMeta = computed(() => {
    ...
    return (
        availableModels.value.find((item) => item.id === currentModel.value) ??
        null
    );
});
...
const selectedOutsoleMeta = computed(() => {
    return (
        outsoleOptions.find((item) => item.key === selectedOutsole.value) ??
        null
    );
});
...
const selectedMidsoleMeta = computed(() => {
    return (
        midsoleOptions.find((item) => item.key === selectedMidsole.value) ??
        null
    );
});
...
const selectedInsoleMeta = computed(() => {
    return (
        insoleOptions.find((item) => item.key === selectedInsole.value) ?? null
    );
});
...
const currentStepMeta = computed(() => {
    return (
        wizardSteps.find((item) => item.id === currentStep.value) ??
        wizardSteps[0]
    );
});
...
const randomPalette = computed(() => {
    const selected = uploadedMedia.value.find(
        (item) => item.id === selectedUploadId.value,
    );
    ...
    if (
        activeElement.value &&
        activeElement.value.type === 'image' &&
        activeElement.value.sourceId
    ) {
        const activeSourceId = activeElement.value.sourceId;
        const fromActive = uploadedMedia.value.find(
            (item) => item.id === activeSourceId,
        );
...
function getElementBaseSize(item: CustomElement): {
    width: number;
    height: number;
} {
    if (item.type === 'image') {
...
function getElementScaledSize(item: CustomElement): {
    width: number;
    height: number;
} {
...
function setInitialModelSelection(folders: StudioFolderMeta[]): void {
    ...
    const matchedFolder =
        folders.find((folder) => folder.key === activeFolderKey.value) ??
        folders[0];
    activeFolderKey.value = matchedFolder.key;

    const stillExists = matchedFolder.models.some(
        (item) => item.id === currentModel.value,
    );
    currentModel.value = stillExists
        ? currentModel.value
        : (matchedFolder.models[0]?.id ?? null);
}
...
async function fetchCatalog(refresh = false): Promise<void> {
    ...
    try {
        const endpoint = refresh
            ? '/api/studio-custom/catalog?refresh=1'
            : '/api/studio-custom/catalog';
        ...
        if (!response.ok) {
            throw new Error(
                `Failed to fetch studio catalog: ${response.status}`,
            );
        }
...
function drawPreview(): void {
    ...
    for (const id of layerIds.value) {
        ...
        const fillLayer = drawFilledLayer(
            image,
            layerColors.value[id] ?? '#ffffff',
            CANVAS_SIZE,
            CANVAS_SIZE,
        );
...
async function loadModelAssets(manualRefresh: boolean): Promise<void> {
    ...
    try {
        ...
        const baseCandidates = [
            model.preview_base_file,
            model.main_file,
        ].filter((item): item is string => Boolean(item));

        for (const fileName of baseCandidates) {
            const image = await loadImage(
                buildAssetUrl(folder.key, model.id, fileName),
            );
        ...
        const loadedLayers = await Promise.all(
            model.layers.map(async (layer) => {
                const image = await loadImage(
                    buildAssetUrl(folder.key, model.id, layer.file),
                );
        ...
        patternLayerFilesRef.value = new Map(
            model.pattern_layers.map((item) => [item.id, item.file]),
        );
...
function updateElement(
    id: string,
    updater: (item: CustomElement) => CustomElement,
): void {
...
function applyPaletteToLayers(palette: string[]): void {
    ...
    layerColors.value = layerIds.value.reduce<Record<LayerId, string>>(
        (carry, id) => {
            const color =
...
function removeActiveElement(): void {
    ...
    customElements.value = customElements.value.filter(
        (item) => item.id !== activeElementId.value,
    );
    activeElementId.value = null;
}
...
async function removeActiveImageBackground(): Promise<void> {
    ...
    updateActiveImage({
        ...
        naturalHeight:
            nextImg?.naturalHeight ?? activeElement.value.naturalHeight,
    });
...
function getTextElementStyle(item: TextElement): CSSProperties {
    ...
    (style as Record<string, string>).WebkitTextStroke =
        `${item.strokeSize * scale.value}px ${item.strokeColor}`;
...
function handlePointerMove(event: PointerEvent): void {
    ...
    const dy =
        ((event.clientY - drag.startClientY) * CANVAS_SIZE) / rect.height;
...
function beginTransformInteraction(
    ...
    const centerClientX =
        rect.left + (centerCanvasX * rect.width) / CANVAS_SIZE;
    const centerClientY =
        rect.top + (centerCanvasY * rect.height) / CANVAS_SIZE;
...
function handleTransformPointerMove(event: PointerEvent): void {
    ...
    if (state.mode === 'scale') {
        const ratio = clampValue(
            Math.hypot(dx, dy) / state.startDistance,
            0.2,
            3.2,
        );
...
async function drawElementsToCanvas(
    ...
    for (const element of customElements.value) {
        if (element.type === 'image') {
            ...
            const imageHeight =
                element.size *
                (element.naturalHeight / Math.max(1, element.naturalWidth));
            ...
            if (mirror) {
                const mirrored = getMirrorTopLeft(
                    x,
                    y,
                    drawW,
                    drawH,
...
async function createPatternCanvas(): Promise<{
    ...
    const patternBase = patternBaseFileRef.value
        ? await loadImage(
              buildAssetUrl(folder.key, model.id, patternBaseFileRef.value),
          )
        : null;
    ...
    if (patternBase) {
        const cutlineOutline = drawOutlineLayer(
            patternBase,
            '#000000',
            15,
            targetWidth,
    ...
    for (const id of layerIds.value) {
        ...
        const fillCanvas = drawFilledLayer(
            image,
            layerColors.value[id] ?? '#ffffff',
            targetWidth,
            targetHeight,
...
async function handleSave(): Promise<void> {
    if (
        !selectedOutsole.value ||
        !selectedMidsole.value ||
        !selectedInsole.value
    ) {
        currentStep.value = 1;
        window.alert(
            'Mohon pilih material outsole, midsole, dan insole sebelum checkout.',
        );
    ...
    if (
        ...
        window.alert(
            'Mohon isi data wajib guest checkout: nama, WhatsApp, email, ukuran, dan alamat.',
        );
    ...
    try {
        ...
        showToast(
            'Guest checkout berhasil diproses. File preview dan pola telah diunduh.',
        );
    } catch (error) {
        console.error('Failed to save design:', error);
        window.alert(
            'Terjadi kesalahan saat memproses desain. Silakan coba lagi.',
        );
    } finally {
