...
const { pushState, undo: undoHistory, redo: redoHistory, isRestoring, clearHistory } = useStudioHistory();
...
const undo = async () => {
    ...
    if (prevState) await restoreState(prevState);
    else showToast('Tidak ada perubahan untuk di-undo');
};

const redo = async () => {
    const nextState = redoHistory();
    if (nextState) await restoreState(nextState);
    else showToast('Tidak ada perubahan untuk di-redo');
};
    ...
    try {
        ...
        for (const elData of state.elements) {
            ...
            if (elData.className === 'Text') {
                ...
                if (attrs.imageSrc) attrs.image = await loadImage(attrs.imageSrc);
...
watch(activeElement, (newEl) => {
    if (!newEl || isRestoring.value) return;
    ...
    if (newEl.type === 'text') {
        ...
        if (textNode.text() !== newEl.text) { textNode.text(newEl.text || ''); changed = true; }
        if (textNode.fontFamily() !== newEl.fontFamily) { textNode.fontFamily(newEl.fontFamily || 'Lexend'); changed = true; }
        if (textNode.fill() !== newEl.color) { textNode.fill(newEl.color || '#000000'); changed = true; }
        if (textNode.stroke() !== newEl.strokeColor) { textNode.stroke(newEl.strokeColor || null); changed = true; }
        if (textNode.strokeWidth() !== (newEl.strokeWidth || 0)) { textNode.strokeWidth(newEl.strokeWidth || 0); changed = true; }
...
watch(activeSideTab, (newTab) => {
    if (newTab === 'checkout' && !hasSeenAddonPopups.value) showFastTrackAlert.value = true;
});
</script>
